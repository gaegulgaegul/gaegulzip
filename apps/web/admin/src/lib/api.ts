...
/**
 * 어드민 공지사항 API 클라이언트
 * NEXT_PUBLIC_SERVER_URL로 Express 서버에 직접 호출
 */
export const noticeApi = {
  async list(page = 1, limit = 20): Promise<NoticeListResponse> {
    ...
    const res = await fetch(`${SERVER_URL}/admin/notices?${params}`);
    if (!res.ok) throw new Error('Failed to fetch notices');
    return res.json();
  },

  async getById(id: number): Promise<Notice> {
    const res = await fetch(`${SERVER_URL}/admin/notices/${id}`);
    if (!res.ok) throw new Error('Failed to fetch notice');
    return res.json();
  },

  async create(data: CreateNoticeData): Promise<Notice> {
    const res = await fetch(`${SERVER_URL}/admin/notices`, {
      method: 'POST',
  ...
  async togglePin(id: number, isPinned: boolean): Promise<void> {
    const res = await fetch(`${SERVER_URL}/admin/notices/${id}/pin`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
