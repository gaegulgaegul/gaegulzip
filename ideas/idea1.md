...
사용자가 사진을 업로드하면, 분석이 시작되기 전에 영화 "관상"의 명장면을 패러디한 인트로 애니메이션/3D 씬을 보여준다.

**원본 장면 (영화 "관상", 2013)**:
- 수양대군(이정재)이 연회장에서 관상가 김내경(송강호)을 발견
- 칼끝으로 가리키며 격하게 반가워하며 다가감
- "어찌, 내가 왕이 될 상인가?" 라고 묻는 장면
...
#### 옵션 A: CSS/SVG 2D 애니메이션
- Al Murphy 스타일의 손그림 일러스트 캐릭터 (사극 관상가)
- SVG 애니메이션으로 캐릭터가 두루마리를 펼치고, 사진을 들여다보는 모션
- 말풍선이 pop-in 되며 "어찌, 내가 탈모가 될 상인가?" 텍스트 표시
- 장점: 가볍고 빠름, Al Murphy 디자인과 톤 일치
- 기술: CSS keyframes + SVG animate / Framer Motion

#### 옵션 B: Three.js 3D 씬
- 로우폴리(low-poly) 스타일의 3D 관상가 캐릭터
- 책상 위에 두루마리와 촛불이 놓인 사극 점술 방 배경
- 사용자 사진이 두루마리 위에 텍스처로 매핑됨
- 카메라가 천천히 줌인하며 관상가가 사진을 살펴보는 연출
- 말풍선 또는 자막으로 "어찌, 내가 탈모가 될 상인가?" 표시
- 장점: 몰입감 높음, 임팩트 강함
- 기술: Three.js + React Three Fiber (@react-three/fiber, @react-three/drei)

#### 옵션 C: Lottie/Rive 하이브리드 애니메이션
- After Effects 또는 Rive에서 제작한 고퀄리티 캐릭터 애니메이션
- 사극풍 관상가가 사진을 받아들고 → 표정 변화 → 대사 말풍선
- 인터랙티브 요소: 사용자가 탭/클릭하면 다음 단계로 진행
- 장점: 가장 부드러운 모션, 프로페셔널한 퀄리티
- 기술: Lottie (lottie-react) 또는 Rive (@rive-app/react-canvas)

**공통 연출 요소:**
- 사극풍 BGM 또는 효과음 (북소리, 바람소리 등) — 음소거 기본, 토글 가능
- 대사 텍스트에 타이핑 효과 (한 글자씩 나타남)
- 업로드한 사진이 두루마리/족자 안에 자연스럽게 합성되는 연출
- 전체 인트로 길이: 3~5초 (Skip 버튼 제공)

**캐릭터 디자인 가이드:**
- Al Murphy 일러스트 스타일을 기반으로 한 사극 관상가 캐릭터
- 갓을 쓰고 도포를 입은 조선시대 관상가
- 과장된 표정 (눈을 크게 뜨거나, 수염을 만지작거리거나)
- 손그림 느낌의 라인 + 컬러풀한 채색
- 캐릭터는 앱 전체에서 마스코트로 재활용 (결과 카드, 에러 화면 등)